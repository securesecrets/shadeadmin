...
pub trait MultiTestable<>
{
    ...
    fn init<T: Serialize>(router: &mut App, sender: Addr, label: &str, send_funds: &[Coin], msg: &T) -> ContractInfo {
    ...
    fn execute<T: Serialize + std::fmt::Debug>(&self, router: &mut App, sender: Addr, msg: &T, send_funds: &[Coin],) -> AnyResult<AppResponse> {
