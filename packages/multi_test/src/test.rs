...
use shade_admin::admin::{
    AdminAuthError, AdminAuthStatus, ConfigResponse, ExecuteMsg, InstantiateMsg, QueryMsg,
    RegistryAction, ValidateAdminPermissionResponse,
};
use shade_protocol::{
    c_std::{Addr, StdResult},
    multi_test::App,
...
fn basic_admin_test() {
    ...
    let resp: ConfigResponse = QueryMsg::GetConfig {}
    ...
    assert!(err.to_string().contains(
        &AdminAuthError::UnregisteredAdmin {
            user: owner.clone()
        }
        .to_string()
    ));
    ...
    ExecuteMsg::ToggleStatus {
        new_status: AdminAuthStatus::Shutdown,
    }
    .test_exec(&mock_admin, &mut router, super_admin.clone(), &[])
    .unwrap();
    ...
    assert!(resp
        ...
        .unwrap()
        .to_string()
        .contains(&AdminAuthError::IsUnderMaintenance.to_string()));

    ExecuteMsg::ToggleStatus {
        new_status: AdminAuthStatus::Active,
    }
    .test_exec(&mock_admin, &mut router, super_admin.clone(), &[])
    .unwrap();

    let resp: ValidateAdminPermissionResponse = QueryMsg::ValidateAdminPermission {
        permission: perm_1.clone(),
        user: owner.to_string(),
    }
    .test_query(&mock_admin, &router)
